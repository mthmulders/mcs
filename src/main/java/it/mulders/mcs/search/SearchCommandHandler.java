...
public class SearchCommandHandler {
  ...
  private SearchResponse.Response performAdditionalSearch(
    ...
    var updatedQuery =
        query.toBuilder().withStart(lastItemFoundIndex).withLimit(remainingLimit).build();
  ...
  private void processResponse(
      final SearchQuery query, final SearchResponse.Response searchResponse) {
    if (showVulnerabilities) {
      reportClient
          .search(searchResponse.docs())
          .ifPresentOrElse(
              componentResponse ->
                  processComponentReports(
