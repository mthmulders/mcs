...
class SearchClientIT implements WithAssertions {
  ...
  class WildcardSearchTest {
    ...
    void should_parse_response(final WireMockRuntimeInfo wmRuntimeInfo) {
      // Arrange
      stubFor(
          get(urlPathMatching("/solrsearch/select*"))
              .willReturn(ok(getResourceAsString("/wildcard-search-response.json"))));

      // Act
      var result =
          new SearchClient(wmRuntimeInfo.getHttpBaseUrl())
              .search(
                  new WildcardSearchQuery(
                      "plexus-utils",
                      Constants.DEFAULT_MAX_SEARCH_RESULTS,
                      Constants.DEFAULT_START));
      ...
      var ids =
          Arrays.stream(result.value().response().docs())
  ...
  class ErrorHandlingTest {
    ...
    void should_gracefully_handle_4xx_response(final WireMockRuntimeInfo wmRuntimeInfo) {
      ...
      assertThat(result.cause())
          .hasMessageContaining("https://github.com/mthmulders/mcs/discussions");
    }
    ...
    void should_gracefully_handle_connection_failure() {
      ...
      var result =
          new SearchClient("http://localhost:21")
              .search(
                  new WildcardSearchQuery(
                      "plexus-utils",
                      Constants.DEFAULT_MAX_SEARCH_RESULTS,
